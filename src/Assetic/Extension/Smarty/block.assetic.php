    ...
    if ($repeat) {
		...
		if ($params['debug']) {
	    	...
	    	foreach ($asset as $a) {
				...
				$writer->writeAsset($cache);
				$assets_urls[] = $a->getTargetPath();
			}
			// It's easier to fetch the array backwards, so we reverse it to insert assets in the right order
			$assets_urls = array_reverse($assets_urls);
			
			$count = count($assets_urls);
			
			$template->assign($params['asset_url'], $params['build_path'].'/'.$assets_urls[$count-1]);
		}
